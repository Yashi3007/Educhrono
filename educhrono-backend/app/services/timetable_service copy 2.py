...
def generate_timetable():
    try:
        ...
        for (sem, section), subjects in grouped.items():
            ...
            print(f"Subjects: {[r['Subject_Name'] for r in subjects]}")
        ...
        # ===============================================================
        # 📘 TUTORIALS (Always 2 groups)
        # ===============================================================
        tutorial_subjects = [
            r for r in subjects
            if int(r.get("T", 0)) > 0
            and int(r.get("COE", 0)) == 0
            and int(r.get("PDP", 0)) == 0
        ]
        ...
        for row in tutorial_subjects:
            ...
            # Choose an alternate teacher for 2nd batch
            ...
            for _ in range(int(row.get("T", 1))):
                ...
                for _ in range(200):
                    ...
                    slot_combined = f"{s1}+{s2}"

                    # Skip lunch or conflict
                    if lunch_slot in [s1, s2]:
                        continue
                    if is_conflict(faculty, section, day, slot_combined):
                        continue
                    if is_conflict(alternate["Faculty_Name"], section, day, slot_combined):
                        continue

                    # ✅ Add both groups together (A1 + A2)
                    timetable.append({
                        "faculty": faculty,
                        "faculty_code": fcode,
                        "subject": subj,
                ...
                if not placed:
                    print(f"⚠️ Could not place TUTORIAL for {subj} ({section})")


        # ===============================================================
        # 🧪 PRACTICALS (Always 2 groups)
        # ===============================================================
        practical_subjects = [
            r for r in subjects
            if int(r.get("P", 0)) > 0
        ...
        for row in practical_subjects:
            ...
            for _ in range(int(row.get("P", 1))):
                placed = False
                for _ in range(200):
                    ...
                    # ✅ Always create both batches together (A1 + A2)
                    timetable.append({
                        "faculty": faculty,
                        "faculty_code": fcode,
                        "subject": subj,
                ...
                if not placed:
                    print(f"⚠️ Could not place PRACTICAL for {subj} ({section})")
            ...
            timetable = [
                ...
                if t["subject"].strip().upper() not in
                ["PROFESSIONAL DEVELOPMENT PROGRAM", "COMMUNICATION & ETHICS"]
            ]

        # ===============================================================
        # 🎓 COE (2-hour block, section-wide)
        # ===============================================================
        coe_subjects = [r for r in subjects if int(r.get("COE", 0)) > 0]
        print(f"📘 Found {len(coe_subjects)} COE subjects for section {section}")
        ...
        for row in coe_subjects:
            ...
            fcode = row["Faculty_Code"]
            dept = row["Department"]
            count = 3   # ✅ Force exactly 3 classes per week
            used_days = set()

            for _ in range(count):
                placed = False
                for _ in range(200):
                    # Ensure COE not on same day as PDP
                    pdp_days = {t["day"] for t in timetable if t["section"] == section and t["type"] == "PDP"}

                    # Pick a day not already used for COE or PDP
                    available_days = [d for d in days if d not in used_days and d not in pdp_days]
                    if not available_days:
                        print(f"⚠️ No available days left for COE ({section})")
                        break

                    day = random.choice(available_days)
                    s1, s2 = "2:00–3:00", "3:00–4:00"  # Always afternoon
                    print(f"🧩 Trying to place COE on {day} ({s1}+{s2})")

                    # Avoid section conflict for that day
                    section_conflict = any(
                        entry["day"] == day and entry["section"] == section
                        and any(s in entry["time_slot"] for s in [s1, s2])
                        for entry in timetable
                    )
                    if section_conflict:
                        print(f"⚠️ COE conflict on {day}, retrying…")
                        continue
                    ...
                    used_days.add(day)
                    placed = True
                    print(f"✅ COE placed successfully for {subj} on {day} ({s1}+{s2})")
                    break

                if not placed:
                    print(f"⚠️ Could not place COE class {COE_count+1} for {subj} ({section})")


        # ===============================================================
        # 🧠 PDP (1-hour, morning only)
        # ===============================================================
        pdp_subjects = [r for r in subjects if int(r.get("PDP", 0)) > 0]
        print(f"📗 Found {len(pdp_subjects)} PDP subjects for section {section}")
        ...
        for row in pdp_subjects:
            ...
            for _ in range(count):
                placed = False
                for _ in range(200):
                    # Avoid PDP and COE on same day
                    coe_days = {t["day"] for t in timetable if t["section"] == section and t["type"] == "COE"}
                    available_days = [d for d in days if d not in used_days and d not in coe_days]
                    if not available_days:
                        print(f"⚠️ No available days left for PDP ({section})")
                        break

                    day = random.choice(available_days)
                    slot = random.choice(["9:00–10:00", "10:00–11:00", "11:00–12:00"])
                    if slot == lunch_slot:
                        continue
                    ...
                    used_days.add(day)
                    placed = True
                    print(f"✅ PDP placed successfully for {subj} on {day} ({slot})")
                    break

                if not placed:
                    print(f"⚠️ Could not place all PDP classes for {subj} ({section})")
