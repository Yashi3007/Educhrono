...
// 🧾 Download Student Timetable as PDF (Single Page + Signatures)
const downloadPDF = async () => {
  const input = tableRef.current;
  if (!input) return;

  // Clone timetable element for clean rendering
  ...
  try {
    ...
    // 🧩 Adjust to fit within single page (leave space for footer + signatures)
    ...
    // ✍️ Signature Lines (same as Faculty)
    const leftX = 35;
                        ...
                        if (twoHourEntries.length > 0) {
                          ...
                          return (
                            <td
                              ...
                              colSpan={2}
                              className={`border border-gray-400 p-2 whitespace-pre-line text-sm text-gray-800 ${
                                typeColor[twoHourEntries[0].type]
                              }`}
                            >
                          ...
                          >
                            {oneHourEntries.map((entry, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <span className="block font-semibold text-gray-900">
                                    {iconMap[entry.type] || ""} {entry.subject}{" "}
                                    ({entry.type}
                                    {entry.batch ? ` - ${entry.batch}` : ""})
                                  </span>
                                  <span className="block text-gray-700">
